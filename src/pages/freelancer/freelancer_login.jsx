...
const FreelancerLogin = () => {
    ...
    useEffect(() => {
        if (import.meta.env.DEV) {
            setLockedUntil(null);
            setAttempts(0);
            return; // Não restaura estado em dev
        }
        const stored = localStorage.getItem('freelancer_login_lock');
        if (stored) {
    ...
    useEffect(() => {
        if (import.meta.env.DEV) return; // Não persiste bloqueio em dev
        if (lockedUntil) {
            localStorage.setItem('freelancer_login_lock', JSON.stringify({
                lockedUntil,
                attempts,
            }));
        } else {
            localStorage.removeItem('freelancer_login_lock');
        }
        ...
        <div className="registration-page">
            <div className="registration-container login-horizontal">
                <div className="login-visual">
                    ...
                    <p className="registration-subtitle">Acesse sua conta para gerenciar projetos e contratos.</p>
                </div>
                <form id="freelancer-login-form" className="registration-form login-form" onSubmit={handleSubmit}>
